...
new class extends Component {
    ...
    public function duplicateComponent($index)
    {
        ...
        // $this->successMessage = 'Composant dupliqué avec succès !';
        // $this->dispatch('reset-messages');
    }
};
?>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-8">
        ...
        <div class="max-w-7xl mx-auto mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Créateur de Formulaires</h1>
                    <p class="text-gray-600 dark:text-gray-400">Glissez et déposez des composants pour créer votre formulaire personnalisé</p>
                </div>
                <div class="flex items-center space-x-3">
                    <button class="px-6 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 border border-gray-200 dark:border-gray-700">
                    ...
                    <button class="px-6 py-2.5 bg-blue-600 text-white rounded-lg shadow-sm hover:shadow-md hover:bg-blue-700 transition-all duration-200">
                        <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="space-y-2 max-h-[calc(100vh-200px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                            @foreach($availableComponents as $component)
                                <button 
                                    ...
                                    class="w-full flex items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 dark:hover:border-blue-700 border-2 border-transparent transition-all duration-200 group">
                                    <div class="w-10 h-10 flex items-center justify-center bg-white dark:bg-gray-800 rounded-lg shadow-sm group-hover:shadow-md transition-shadow mr-3">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $component['label'] }}</span>
                                </button>
                            @endforeach
                        </div>
                ...
                <div class="col-span-3">
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 min-h-[600px]">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span class="text-sm text-gray-500 dark:text-gray-400">{{ count($questions) }} composant(s)</span>
                        </div>
                                    ...
                                    <div 
                                        ...
                                        <div class="mb-3">
                                            ...
                                            @if($question['type'] === 'text_input')
                                                <input type="text" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ $question['properties']['placeholder'] ?? 'Entrez du texte' }}" disabled>
                                            
                                            @elseif($question['type'] === 'textarea')
                                                <textarea rows="{{ $question['properties']['rows'] ?? 4 }}" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ $question['properties']['placeholder'] ?? 'Entrez votre texte' }}" disabled></textarea>
                                            
                                            @elseif($question['type'] === 'number_input')
                                                <input type="number" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ $question['properties']['placeholder'] ?? 'Entrez un nombre' }}" disabled>
                                            
                                            @elseif($question['type'] === 'email')
                                                <input type="email" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ $question['properties']['placeholder'] ?? 'exemple@email.com' }}" disabled>
                                            
                                            @elseif($question['type'] === 'phone')
                                                <input type="tel" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ $question['properties']['placeholder'] ?? '+237 6XX XXX XXX' }}" disabled>
                                            
                                            @elseif($question['type'] === 'dropdown')
                                                <select class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" disabled>
                                                    <option>{{ $question['properties']['placeholder'] ?? 'Sélectionnez une option' }}</option>
                                                </select>
                                            
                                            @elseif($question['type'] === 'checkbox')
                                                <div class="flex items-center">
                                                    <input type="checkbox" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" disabled>
                                            ...
                                            @elseif($question['type'] === 'radio')
                                                <div class="space-y-2">
                                                    @foreach(explode(',', $question['properties']['options'] ?? 'Option 1,Option 2') as $option)
                                                        <div class="flex items-center">
                                                            <input type="radio" name="radio-{{ $index }}" class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500" disabled>
                                            ...
                                            @elseif($question['type'] === 'date_picker')
                                                <input type="date" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" disabled>
                                            ...
                                            @elseif($question['type'] === 'file_upload')
                                                <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
                                                    ...
                                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Cliquez pour télécharger ou glissez un fichier</p>
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="absolute top-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button 
                                                ...
                                                class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all">
                                            ...
                                            <button 
                                                ...
                                                class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-all">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 sticky top-8">
                        ...
                        @if($selectedComponent && $selectedIndex !== null && isset($questions[$selectedIndex]))
                            <div class="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                                ...
                                <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                    ...
                                    <label for="required-checkbox" class="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300">Champ obligatoire</label>
                                </div>
                                ...
                                @if(in_array($questions[$selectedIndex]['type'], ['dropdown', 'radio']))
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Options (séparées par virgule)</label>
                                        <textarea 
                                ...
                                <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                    <div class="flex items-start">
                                        ...
                                        <div>
                                            <p class="text-xs font-medium text-blue-900 dark:text-blue-300">Type: {{ ucfirst(str_replace('_', ' ', $questions[$selectedIndex]['type'])) }}</p>
                                            <p class="text-xs text-blue-700 dark:text-blue-400 mt-1">Modifiez les propriétés pour mettre à jour le composant</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @else
                            <div class="text-center py-12">
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">Sélectionnez un composant dans le formulaire pour modifier ses propriétés</p>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <!-- JavaScript for Message Timeout -->
        <script>
